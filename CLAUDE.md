...
For a future start, `buildSchedule`'s third argument `from` widens the face range backwards so the hours between now and `start` become **faces with no wedges**. That keeps the carousel sitting on the real current hour — an empty clock with live hands — and lets the first block hour arrive from the upcoming slot and slide into the centre when it actually starts, instead of parking a future hour in the centre from the moment the page opens.
...
- [src/components/ClockCarousel.tsx](src/components/ClockCarousel.tsx) keeps every face mounted and positions each by its offset from the current index (`translateX(offset * 38%)` plus a scale/opacity drop for the side), so advancing the index slides the row across. **Only the current hour and the next one are visible** — past hours sit at opacity 0, staying mounted purely so they slide off to the left and fade rather than popping out. Everything on screen is now or still to come.
- [src/components/Legend.tsx](src/components/Legend.tsx) shows the running block with a countdown, then the remaining blocks rolling along underneath.
