...
- **Wedges are static for the whole hour.** [ClockCarousel](src/components/ClockCarousel.tsx) draws `clock.wedges` as built, untouched by `now` — a face keeps its full colors for its entire hour, including time already elapsed, and doesn't lose a wedge until the carousel drops the whole face once its hour is over. Progress through a block is read off the hands sweeping across the disc, not from color disappearing.
- Faces in the future keep their full wedges too, so a face always reads exactly like a real wall clock at that hour — current and future faces are drawn identically.
- [src/hooks/useNow.ts](src/hooks/useNow.ts) re-renders every 30 seconds (`useNow(30_000)` in [Clock.tsx](src/pages/Clock.tsx)) to move the hands, traded down from once a second to keep the ticking page cheap. `Settings.tsx` uses the same 30s default for its own live preview.
- [src/components/ClockFace.tsx](src/components/ClockFace.tsx) draws one face as SVG in a 100×100 viewBox: numbers and ticks around the outside, colored wedges as an inner Time Timer style disc (drawn full, not trimmed to `now`), and live hour + minute hands (no second hand). **Hands render only on the centre face when that face actually contains `now`** (`isCurrent && containsTime(clock, now)`) — being centred isn't enough, since a future start or a finished schedule centres the nearest face rather than the one `now` falls in, and real hands there would point at a time outside that hour and mislead a child. Hands are drawn twice, a `var(--bg)` halo under a `var(--text-h)` stroke, so they stay readable over any wedge color.
